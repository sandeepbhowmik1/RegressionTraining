...
Production version (training with full sample)
...
inputFiles = []
inputFiles.extend(glob.glob("/data_CMS/cms/sauvan/Regression/Ntuples/DoubleElectron_FlatPt-1To300/regression_ntuple_RunIISpring15DR74-AsymptFlat0to50bx25RawReco_MCRUN2_74_V9A-v2_2015_07_15/regression_ntuple_746_*.root"))
...
batch.baseName = "GBRLikelihood_Clustering_746_bx25_Electrons_PROD"
...
batch.outputDirectory = "/data_CMS/cms/sauvan/RegressionResults/StudyClustering/746/GBRLikelihood_Clustering_bx25_Electrons_MoreVariables_PtCut50_PtSlope50_Sig5_PROD/"
...
batch.commonCuts        = ["(isMatched==1)"]
batch.commonCutsError   = ["(eventNumber%2!=0)&&(((eventNumber-1)/2)%4==3)&&(isMatched==1)"]
batch.commonCutsComb     = ["((eventNumber%2!=0)&&(isMatched==1))"] ## For production (half sample for combination)
...
doCombine = {
    "GedGsfElectron_RegPs1":True,
    "PFMustache_RegPs1":False,
    "GedGsfElectron_RegPs2":True,
    "PFMustache_RegPs2":False,   
}

## half sample if combination. Full sample if no combination
specificCuts = {
    "GedGsfElectron_RegPs1":["(eventNumber%2==0)"],
    "PFMustache_RegPs1":[],
    "GedGsfElectron_RegPs2":["(eventNumber%2==0)"],
    "PFMustache_RegPs2":[],   
}
